...
| Cutom output location + name | The tester will execute the script with a custom file location and name e.g. `~/Desktop/gitout.csv` Both relative and absolute path's are possible | The script will execute normally and palce the output file to the given path. |||
